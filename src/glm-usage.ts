...
export function detectGlmEnv(): GlmEnv | null {
...
function readCache(ttlMs: number): GlmCacheData | null {
  try {
    const raw = fs.readFileSync(getCachePath(), 'utf-8');
    const cache = JSON.parse(raw) as Partial<GlmCacheData>;
...
function writeCache(data: GlmCacheData): void {
  try {
...
function fetchGlmUsage(baseUrl: string, authToken: string): Promise<unknown> {
...
function extractResetAt(limit: Record<string, unknown>): number | null {
...
function toGlmUsageData(cache: GlmCacheData): GlmUsageData {
...
export function parseGlmResponse(response: unknown): Omit<GlmCacheData, 'fetchedAt'> | null {
  const limits = (response as { data?: { limits?: Array<Record<string, unknown>> } } | null)?.data?.limits;
